...
const verifyToken = require("../middleware/verifyToken"); // Use this middleware
...
// amended post verifyToken
router.post("/", verifyToken, async (req, res) => {
  try {
    console.log("Creating vehicle with user:", req.user);

    // Add userId from token to the vehicle data
    const vehicleData = {
      ...req.body,
      userid: req.user.id, // lowercase to match DB column
    };
...
router.post("/:id/photos", verifyToken, upload.array("photos"), async (req, res) => {
  try {
    const { id } = req.params;
